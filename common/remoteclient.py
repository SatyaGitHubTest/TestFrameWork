#this class defines and implements remote clients
import paramiko
import socket
import os
...
class RemoteClient(object):
    ...
    def __init__(self, hostname, port = '22', username='ansible', key_file=None, password=None):
        #
        ...
        #
        # request a internet protocol socket(AF_INET) with TCP transport(SOCK_STREAM)
        self.sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        self.sock.connect((hostname, 22))
        # build paramiko transport layer and start ssh session on above socket
        self.t = paramiko.transport.Transport(self.sock)
        self.t.start_client()
        #Now we have to resolve the public key, first read all keys from user location
        keys = paramiko.util.load_host_keys(os.path.expanduser('~/Documents/aws/testkey.pem'))
        key = self.t.get_remote_server_key()
        # supposed to check for key in keys, but I don't caree right now to find the right notation
        if key_file is not None:
#            if isinstance(key,paramiko):
            key_file1=open(os.path.expanduser(key_file),'r')
            key_head=key_file1.readline()
            key_file1.seek(0)
            if 'DSA' in key_head:
            ...
            pkey=keytype.from_private_key(key_file1)
